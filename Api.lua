        ...
        if msg.inline_query then
          ...
          if Q.query:match("^-(%d+,%S+)") then
            ...
            if lang == "Fa" then
              ...
              answer(Q.id, "settings", "Menu", chat, "• بخش مورد نظر خود را انتخاب کنید : \n•• تاریخ : " .. Time().date .. "\n•• ساعت : " .. Time().time .."", keyboard)
            elseif lang == "En" then
              keyboard.inline_keyboard = {
                {
                  {
                    text = "[••♧️ Grop settings ♧••]",
        ...
        if msg.callback_query then
          ...
          if having_access(Q.from.id, chat, Q.id) then
            if Q.data:match("UpdateModListFa") then
              result = "لیست مدیران گروه بروزرسانی شد 👤 >>> ♻️"
              Q.data = "ModlistFa:" .. chat
              Canswer(Q.id, result)
          ...
          if having_access(Q.from.id, chat, Q.id) then
            if Q.data:match("UpdateBanListFa") then
              result = "لیست افراد مسدود بروزرسانی شد 🚷 >>> ♻️"
              Q.data = "BanlistFa:" .. chat
              Canswer(Q.id, result)
          ...
          if having_access(Q.from.id, chat, Q.id) then
            if Q.data:match("UpdateFilterListFa") then
              result = "لیست کلمات فیلتر شده بروزرسانی شد 💬 >>> ♻️"
              Q.data = "FilterListFa:" .. chat
              Canswer(Q.id, result)
          ...
          if having_access(Q.from.id, chat, Q.id) then
            if Q.data:match("UpdateMuteListFa") then
              result = "لیست افراد بی صدا بروزرسانی شد  🔕 >>> ♻️"
              Q.data = "MutelistEn:" .. chat
              Canswer(Q.id, result)
          ...
          if having_access(Q.from.id, chat, Q.id) then
            if Q.data:match("FirstMenuFa") then
              ...
              edit(Q.inline_message_id, "⚜️ لطفا یکی از دکمه های زیر را لمس کنید ⚜️", keyboard)
            elseif Q.data:match("FirstMenuEn") then
              ...
              edit(Q.inline_message_id, "⚜️ Choose Your Desired Section ⚜️", keyboard)
            end
          end
          if having_access(Q.from.id, chat, Q.id) then
            ...
            elseif Q.data:match("GroupSettingsEn") then
              ...
              edit(Q.inline_message_id, "⚜️ Choose Your Settings Desired Section ⚜️", keyboard)
            end
          end
          if having_access(Q.from.id, chat, Q.id) then
          ...
          if having_access(Q.from.id, chat, Q.id) then
            if Q.data:match("ExitFa") then
              edit(Q.inline_message_id, "✪ فهرست با موفقیت بسته شد !", nil)
