...
if not database:get("setnumbergp") then
  ...
  print("\027[" .. color.black[1] .. ";" .. color.green[2] .. "m\n➡➡ [•• ᴄᴏɴғɪʀᴍᴇᴅ ʟɪᴄᴇɴsᴇ ° ʙʏ MR RAD °  ••]\n\027[00m")

print("\027[" .. color.black[1] .. ";" .. color.white[2] .. "m\n➡➡ [••ᴄʜᴀɴɴᴇʟ: @ActionTG | ɪs sᴇɴᴅ ° ᴜᴘᴅᴇᴛᴇ ° ᴍᴏᴅʀɴ••] ▶\n\027[00m")

print("\027[" .. color.black[1] .. ";" .. color.white[2] .. "m\n➡➡ [••برای دریافت اپدیت ها  به کانال ما بزنید | ᴄʜᴀɴɴᴇʟ: @ActionTG ••] ▶\n\027[00m")
...
> <b>Phone Number</b> : ]] .. phonenumbere, 1, "html")
   ...
   if is_momod(msg.sender_user_id_, msg.chat_id_) and (text:match("^[Pp]ing$") or text:match("^پینگ$")) then
          if database:get("lang:gp:" .. msg.chat_id_) then
            send(msg.chat_id_, msg.id_, 1, "im online but i dont care !!!", 1, "md")
          else
            send(msg.chat_id_, msg.id_, 1, "آنلاینم ولی به تخمم نیست !", 1, "md")
          end
        end
...
`
        ...
        if is_momod(msg.sender_user_id_, msg.chat_id_) and (text:match("^[Ss]etlang (.*)$") or text:match("^تنظیم زبان (.*)$")) then
          ...
          if langs[2] == "fa" or langs[2] == "فارسی" then
            if not database:get("lang:gp:" .. msg.chat_id_) then
              send(msg.chat_id_, msg.id_, 1, "✪↫ خروجی دستورات از قبل به روی زبان `فارسی` قرار گرفته بود  !", 1, "md")
            else
              send(msg.chat_id_, msg.id_, 1, "✪↫ خروجی دستورات به روی زبان `فارسی` قرار گرفت !", 1, "md")
...
All your *Warnings* Has Been Cleared !]], 1, "md")
        ...
        if is_admin(msg.sender_user_id_) and text:match("^[Nn]ote (.*)$") or text:match("^یادداشت (.*)$") then
          local txt = {
        ...
        if is_admin(msg.sender_user_id_) and text:match("^[Gg]etnote$") or text:match("^یادداشت ها$") then
        ...
        if is_admin(msg.sender_user_id_) and idf:match("-100(%d+)") and text:match("^[Cc]harge (%d+)$") or text:match("^شارژ (%d+)$") then
          do
            local a = {
        ...
        if is_admin(msg.sender_user_id_) and idf:match("-100(%d+)") and text:match("^[Cc]harge [Uu]nit$") or text:match("^شارژ نامحدود$") then
        ...
        if is_admin(msg.sender_user_id_) and idf:match("-100(%d+)") and text:match("^[Cc]harge [Tt]rial$") or text:match("^شارژ آزمایشی$") then
...
Credit : *]] .. d .. "* Days !", 1, "md")
        ...
        if is_admin(msg.sender_user_id_) and idf:match("-100(%d+)") and text:match("^[Aa]dd$") or text:match("^فعال شو$") then
          local adding = function(extra, result)
            ...
            if database:get("bot:enable:" .. msg.chat_id_) then
              if not database:get("lang:gp:" .. msg.chat_id_) then
                send(msg.chat_id_, msg.id_, 1, "• گروه " .. (chat and chat.title_ or "") .. " از قبل در لیست گروه های امن ربات میباشد !", 1, "md")
              else
...
If you *want* to get the bot phone number , send command *Botphone* .]], 1, "md")
                ...
                send(msg.chat_id_, msg.id_, 1, "• گروه " .. (chat and chat.title_ or "") .. "گروه به لیست گروه های امن ربات افزوده شد !", 1, "md")
              end
              local fname = result.first_name_ or ""
              local lname = result.last_name_ or ""
        ...
        if text:match("^[Rr]uadmin$") and is_sudo(msg) or text:match("^ادمینی$") and is_sudo(msg) then
